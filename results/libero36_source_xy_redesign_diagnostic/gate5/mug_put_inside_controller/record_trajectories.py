...
OUTPUT = Path(
    "/tmp/libero36_gate5_mug_tasks12_14_formal"
)
SUMMARY = OUTPUT / "summary.csv"
RIM_OFFSET_Z_M = 0.059
...
selected_rows = sorted(
    [
        ...
        if row["task_id"] in {12, 13, 14}
        and row["layout_id"] == 1
    ],
    key=lambda row: row["task_id"],
)

if [row["task_id"] for row in selected_rows] != [12, 13, 14]:
    raise RuntimeError(
        "Expected exactly tasks 12, 13, and 14 on layout 1"
    )
