...
selected_rows = sorted(
    [
        ...
        if row["task_id"] in {9, 10, 11}
        and row["layout_id"] == 1
    ],
    key=lambda row: row["task_id"],
)

if [row["task_id"] for row in selected_rows] != [9, 10, 11]:
    raise RuntimeError(
        "Expected exactly tasks 9, 10, and 11 on layout 1"
    )
