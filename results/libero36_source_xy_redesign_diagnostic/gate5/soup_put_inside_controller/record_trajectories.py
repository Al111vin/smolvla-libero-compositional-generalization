...
OUTPUT = Path(
    "/tmp/libero36_gate5_soup_tasks21_23_selected"
)
...
selected_rows = sorted(
    [
        ...
        if row["task_id"] in {21, 22, 23}
        and row["layout_id"] == 1
    ],
    key=lambda row: row["task_id"],
)

if [row["task_id"] for row in selected_rows] != [21, 22, 23]:
    raise RuntimeError(
        "Expected exactly tasks 21, 22, and 23 on layout 1"
    )
