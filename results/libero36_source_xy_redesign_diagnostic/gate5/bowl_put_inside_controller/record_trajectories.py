...
OUTPUT = Path(
    "/tmp/libero36_gate5_bowl_inside_tasks345_formal"
)
...
selected_rows = sorted(
    [
        ...
        if row["task_id"] in {3, 4, 5}
        and row["layout_id"] == 1
    ],
    key=lambda row: row["task_id"],
)

if [row["task_id"] for row in selected_rows] != [3, 4, 5]:
    raise RuntimeError(
        "Expected exactly tasks 3, 4, and 5 on layout 1"
    )
