...
OUTPUT = Path(
    "/tmp/libero36_gate5_soup_tasks18_20_formal"
)
...
selected_rows = sorted(
    [
        ...
        if row["task_id"] in {18, 19, 20}
        and row["layout_id"] == 1
    ],
    key=lambda row: row["task_id"],
)

if [row["task_id"] for row in selected_rows] != [18, 19, 20]:
    raise RuntimeError(
        "Expected exactly tasks 18, 19, and 20 on layout 1"
    )
