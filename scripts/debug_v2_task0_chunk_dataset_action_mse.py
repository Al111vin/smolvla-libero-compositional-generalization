...
from train_smolvla_v2_task0_chunk import build_policy, add_language_tokens, move_batch_to_device


device = "cuda"
ckpt_path = "outputs/smolvla_v2_task0_chunk/checkpoint_final.pt"
data_path = Path(
...
with h5py.File(data_path, "r") as f:
    ...
    obs = demo["obs"]
    total_steps = demo["actions"].shape[0]

    usable_samples = min(num_samples, total_steps - chunk_size + 1)

    agentview = obs["agentview_rgb"][:usable_samples]
    wrist = obs["eye_in_hand_rgb"][:usable_samples]
    joint = obs["joint_states"][:usable_samples]
    ee_pos = obs["ee_pos"][:usable_samples]
    ee_ori = obs["ee_ori"][:usable_samples]
    gripper = obs["gripper_states"][:usable_samples]

    true_actions = []
    for i in range(usable_samples):
        true_actions.append(demo["actions"][i : i + chunk_size])
    true_actions = np.stack(true_actions, axis=0).astype(np.float32)
...
print("V2 TASK0 CHUNK DATASET ACTION MSE CHECK")
print("=======================================")
print("data file:", data_path)
print("demo key:", demo_key)
print("num samples:", usable_samples)
