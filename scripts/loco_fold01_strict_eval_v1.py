...
# Set headless EGL before importing any MuJoCo/robosuite/LIBERO modules.
...
PROTOCOL_VERSION = "v5_loco_fold01_strict_v1"
...
def parse_args():
    ...
    parser.add_argument("--protocol", default="configs/loco_fold01_task00_protocol.json")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument("--diagnostic-n-action-steps", type=int, default=None)
    parser.add_argument("--diagnostic-init-indices", type=int, nargs="+", default=None)
...
def evaluator_fingerprint(require_protocol: bool) -> str:
    ...
    protocol_path = script_path.parent.parent / "configs" / "loco_fold01_task00_protocol.json"
    paths = [script_path, adapter_path]
    if require_protocol or protocol_path.is_file():
...
def validate_args(args, checkpoint: Path):
    ...
    if args.n_action_steps not in (25, 50):
        raise ValueError("Diagnostic evaluator permits n-action-steps=25 or 50")
    ...
    if len(args.task_ids) != 1 or args.heldout_task_id < 0 or args.task_ids[0] < 0:
        raise ValueError("Evaluator requires one non-negative protocol task id")
    if args.mode == "formal":
...
def main():
    ...
    if spec["benchmark_init_indices"] != list(range(50)):
        raise ValueError("Protocol requires benchmark init indices 0..49")
    task_id = int(spec.get("evaluated_task_id", spec.get("eval_task_id", spec.get("heldout_task_id"))))
    args.heldout_task_id = int(spec.get("formal_fold_heldout_task_id", spec.get("heldout_task_id", task_id)))
    args.task_ids = [task_id]
    args.init_indices = [0] if args.smoke else list(spec["benchmark_init_indices"])
    if args.diagnostic_init_indices is not None:
        args.init_indices = args.diagnostic_init_indices
    args.max_steps = int(spec["max_steps"])
    args.wait_steps = int(spec["wait_steps"])
    args.n_action_steps = int(spec["n_action_steps"])
    if args.diagnostic_n_action_steps is not None:
        if args.diagnostic_n_action_steps != 50:
            raise ValueError("Only diagnostic n_action_steps=50 is permitted")
        args.n_action_steps = args.diagnostic_n_action_steps
    args.base_env_seed = 1000
    args.base_policy_seed = 2000
    args.model_label = checkpoint_label(Path(args.checkpoint))
    role = spec.get("role", "formal_heldout" if task_id == args.heldout_task_id else "in_distribution_diagnostic")
    if role == "formal_heldout" and task_id != args.heldout_task_id:
        raise ValueError("formal_heldout role requires evaluated task equal formal held-out task")
    if role == "in_distribution_diagnostic" and task_id == args.heldout_task_id:
        raise ValueError("in_distribution_diagnostic role requires evaluated task differ from formal held-out task")
    args.mode = "sanity" if (args.smoke or args.diagnostic_init_indices is not None or role != "formal_heldout") else "formal"
    ...
    # The isolated evaluation prep directory is intentionally not a Git
    # checkout; the evaluator and protocol hashes provide its provenance.
    ...
    for task_id in args.task_ids:
        ...
        task.language = spec.get("instruction", task.language)
    ...
    manifest = {
        "schema_version": "loco_fold01_task00_run_manifest_v1",
        "evaluation_mode": args.mode,
        "role": role,
        "evaluated_task_id": task_id,
        "heldout_task_id": args.heldout_task_id,
        "trial_count": len(requested_pairs),
