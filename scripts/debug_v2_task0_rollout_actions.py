...
results_path = Path("results/debug_v2_task0_rollout_actions.csv")
max_steps = 300
...
def run_one_task(task_id):
    ...
    action_log_path = "results/debug_v2_task0_rollout_actions.csv"
    action_df.to_csv(action_log_path, index=False)
    ...
    print()
    print("ACTION JUMP MEAN ABS DIFF:")
    action_cols = [f"action_{i}" for i in range(7)]
    print(action_df[action_cols].diff().abs().mean())

    print()
    print("SAVED ACTION LOG TO:", action_log_path)
