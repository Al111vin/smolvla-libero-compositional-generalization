...
DIAGNOSTIC_PROTOCOL = "libero_36_source_x_candidate_preflight_v1"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Generate and test one predeclared LIBERO-36 source-x "
...
def candidate_protocol(candidate: str) -> str:
    return f"libero_36_source_x_diagnostic_{candidate}_v1"
    ...
    try:
        ...
        for phase, waypoint, position_cap in waypoints:
            obs, reached, diagnostics = push_calibrator.move_pad_to(
                ...
                f"reach_{phase}",
                push_calibrator.MAX_TRANSIT_STEPS,
                position_cap,
            )
...
def main():
    ...
    print("=" * 80)
    print("LIBERO-36 source-x candidate preflight")
    ...
    write_json(
        candidate_root / "preflight_manifest.json",
        {
            ...
            "selection_rule": (
                "rightmost-first predeclared order c0,c1,c2; advance only "
                "after a preceding candidate fails"
            ),
            "phase": args.phase,
