...
OUTPUT = Path(os.environ.get("LIBERO_OUTPUT_ROOT", "/tmp/libero36_task23_seed"))
...
selected_rows = sorted(
    [
        ...
        if row["task_id"] in {23}
        and row["layout_id"] == 1
    ],
    key=lambda row: row["task_id"],
)

if [row["task_id"] for row in selected_rows] != [23]:
    raise RuntimeError("Expected exactly task 23 on layout 1")


def run_one(row, rim_offset_x_m, close_steps, seed_override=None):
    task_id = int(row["task_id"])
    seed = int(seed_override) if seed_override is not None else validator.seed_for(row, calibrator.GATE3_CALIBRATION_RESET_INDEX, validator.FULL_SEED_BASE)
    ...
    episode_done = False
    current_phase = "settle_environment"
    ...
    try:
        ...
        basket_geoms = calibrator.collision_geom_names(
            env,
            "basket_1",
        )
        ...
        def raw_step(action, phase):
            nonlocal obs, episode_done, current_phase
            current_phase = phase
            ...
            if episode_done:
                ...
                try:
                    obs, reward_value, done, _ = env.step(action)
                except ValueError as exc:
                    if "terminated episode" in str(exc):
                        episode_done = True
                        print(f"EARLY_TERMINATION phase={phase} actions={metrics['actions']}", flush=True)
                        return
                    raise
                relation = bool(env.check_success())
                # LIBERO reports done=True when the task relation succeeds,
                # but the validated controller intentionally continues the
                # terminal-hold trace. Only a non-success terminal state
                # must block subsequent env.step calls.
                if done and not relation:
                    episode_done = True

            if bool(done) and not relation:
                # Preserve an early non-success termination as a failed
                # trajectory; never call env.step after termination.
                episode_done = True
            if not done and bool(done) != relation:
                raise RuntimeError(
                    f"done mismatch in {phase}: "
                    f"done={done}, relation={relation}"
                )

            # Persist the observation produced by this action.  On success
            # LIBERO may report done=True while still allowing terminal-hold
            # steps; those remain part of the validated trajectory.
            frame_actions.append(action.copy())
            frame_rewards.append(float(locals().get("reward_value", 0.0)))
            frame_dones.append(bool(done))
            frame_obs["agentview_rgb"].append(np.asarray(obs["agentview_image"]).copy())
            frame_obs["eye_in_hand_rgb"].append(np.asarray(obs["robot0_eye_in_hand_image"]).copy())
            frame_obs["joint_states"].append(np.asarray(obs["robot0_joint_pos"], dtype=np.float64).copy())
            frame_obs["ee_pos"].append(np.asarray(obs["robot0_eef_pos"], dtype=np.float64).copy())
            frame_obs["ee_ori"].append(np.asarray(transform.quat2axisangle(obs["robot0_eef_quat"]), dtype=np.float64).copy())
            frame_obs["gripper_states"].append(np.asarray(obs["robot0_gripper_qpos"], dtype=np.float64).copy())
            ...
            metrics["robot_basket_steps"] += int(
                robot_basket
            )
            ...
            trace_robot_basket.append(bool(robot_basket))
        ...
        rim_center[2] += RIM_OFFSET_Z_M

        above_rim = rim_center.copy()
        above_rim[2] += 0.130

        if task_id == 23:
            # The direct diagonal route intersects
            # the basket in the right region.
            transit_lift = finger_center().copy()
            transit_lift[2] = max(
                float(transit_lift[2]) + 0.120,
                float(above_rim[2]) + 0.120,
            )

            transit_lift_ok, transit_lift_error = (
                move_to(
                    transit_lift,
                    -1.0,
                    0.20,
            ...
            transit_align = above_rim.copy()
            transit_align[2] = transit_lift[2]

            transit_align_ok, transit_align_error = (
                move_to(
                    transit_align,
                    -1.0,
                    0.20,
        ...
        desired_target_above_basket = np.array(
            [
                basket_xyz[0],
                basket_xyz[1],
                max(
        ...
        checks = {
            ...
            "no_robot_basket_contact": bool(
                metrics["robot_basket_steps"] == 0
            ),
        ...
        np.savez_compressed(
            ...
            robot_basket_contact=np.asarray(
                trace_robot_basket,
                dtype=np.bool_,
            ),
        ...
        hdf5_path = OUTPUT / "task_023_layout_1_dynamic.hdf5"
        with h5py.File(hdf5_path, "w") as h5:
            ...
            data.attrs["problem_info"] = json.dumps({
                "language_instruction": "pick up the alphabet soup and place it inside the basket in the right region",
                "task_id": 23, "layout_id": 1,
            })
            demo = data.create_group("demo_0")
            demo.attrs.update({"task_id": 23, "layout_id": 1, "seed": seed,
                               "passed": True, "success": True, "num_samples": len(frame_actions)})
            demo.create_dataset("actions", data=np.asarray(frame_actions, dtype=np.float32))
            demo.create_dataset("rewards", data=np.asarray(frame_rewards, dtype=np.float32))
            demo.create_dataset("dones", data=np.asarray(frame_dones, dtype=np.uint8))
            demo.create_dataset("states", data=np.asarray(trace_states, dtype=np.float64))
            demo.create_dataset("robot_states", data=np.concatenate([
                np.asarray(frame_obs["joint_states"], dtype=np.float64),
                np.asarray(frame_obs["gripper_states"], dtype=np.float64)], axis=1))
            obs_group = demo.create_group("obs")
            obs_group.create_dataset("agentview_rgb", data=np.asarray(frame_obs["agentview_rgb"], dtype=np.uint8), compression="gzip")
            obs_group.create_dataset("eye_in_hand_rgb", data=np.asarray(frame_obs["eye_in_hand_rgb"], dtype=np.uint8), compression="gzip")
            for name in ("joint_states", "ee_pos", "ee_ori", "gripper_states"):
                obs_group.create_dataset(name, data=np.asarray(frame_obs[name], dtype=np.float64))
        ...
        return {
            ...
            "robot_basket_contact_steps":
                int(metrics["robot_basket_steps"]),
        ...
        return {
            ...
            "passed": False,
            "error": (
                f"{type(exc).__name__}: {exc} (phase={current_phase})"
            ),
        }
...
for row in selected_rows:
    ...
    result["release_height_offset_m"] = 0.140
