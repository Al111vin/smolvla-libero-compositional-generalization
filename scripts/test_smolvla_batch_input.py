...
class LiberoSmolVLABatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        action = torch.from_numpy(action).float().unsqueeze(0)  # (1, 7)
...
def main():
    print("=" * 80)
    print("SmolVLA batch input test")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print("device:", device)

    dataset = LiberoSmolVLABatchDataset(DEMO_PATH)
    ...
    print("=" * 80)
    print("Running policy.forward(batch)...")
    ...
    print("=" * 80)
    print("SmolVLA batch input test passed.")
